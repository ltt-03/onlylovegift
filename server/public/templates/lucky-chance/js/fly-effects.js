...
(function () {
    ...
    function moveGlobalDrag(e) {
        ...
        globalWishScale = clamp(
            globalWishScale - dy * 0.0012,
            GLOBAL_SCALE_MIN,
            GLOBAL_SCALE_MAX
        );
    ...
    function endGlobalDrag(e) {
        ...
        dragPointerId = null;
        updateLayerClasses();
        try { screen.releasePointerCapture(e.pointerId); } catch (_) { /* noop */ }
        dampingAnimation();
    }
    ...
    function applyItemTransform(el) {
        ...
        inner.style.transform =
            `rotateX(${s.rotateX}deg) rotateY(${s.rotateY}deg) scale(${s.scale})`;
    }
    ...
    function startItemDrag(e, fallingItem) {
        ...
        const onUp = (ev) => {
            if (ev.pointerId !== s.pointerId) return;
            s.pointerId = null;
            try { fallingItem.releasePointerCapture(ev.pointerId); } catch (_) { /* noop */ }
