...
function initMusic() {
    const musicUrl = (window.DYNAMIC_DATA && window.DYNAMIC_DATA.music) ? window.DYNAMIC_DATA.music : '/templates/x-mas-tree/style/nhac.mp3';
...
let images = [];
if (window.DYNAMIC_DATA && window.DYNAMIC_DATA.images && window.DYNAMIC_DATA.images.length > 0) {
    images = window.DYNAMIC_DATA.images;
} else {
    for (let i = 1; i <= 30; i++) {
        images.push('/templates/x-mas-tree/style/img/Anh (' + i + ').jpg');
    }
}
...
async function initMediaPipe() {
    try {
        ...
        const vision = await FilesetResolver.forVisionTasks(
            "/templates/x-mas-tree/vendor/mediapipe/wasm"
        );
        handLandmarker = await HandLandmarker.createFromOptions(vision, {
            baseOptions: {
                modelAssetPath: `/templates/x-mas-tree/vendor/mediapipe/model/hand_landmarker.task`,
                delegate: "GPU"
            },
            runningMode: "VIDEO",
            numHands: 1
        ...
        debugInfo.innerText = "MediaPipe error: " + err.message;
        document.getElementById('webcam-wrapper').style.display = 'none';
    }
}
    ...
    if (result.landmarks && result.landmarks.length > 0) {
        if (!musicStarted && bgMusic) {
            bgMusic.play()
                .then(() => {
                    musicStarted = true;
                    console.log("🎵 Music started by hand gesture");
                })
                .catch(() => {});
        }
